sap.ui.define(["sap/ui/core/mvc/Controller", "sap/ui/model/json/JSONModel", "sap/m/MessageBox", "sap/ui/core/Fragment",
	...
	"use strict";
	return e.extend("dksh.connectclient.InventoryEnd.controller.inventory", {
		formatter: o,
		...
		_getUser: function () {
			...
			this._doAjax(url, "GET", "", true).then(success => {
				...
				this.clearTabData();
				this._getPersonalizationDetails("keyEndingStockView");
		...
		onPressDetailEndStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryEnd.Fragments.EndingStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
		...
		onPressDetailExpStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryEnd.Fragments.ExpiryStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailMatMov: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryEnd.Fragments.MatMovPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailATP: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryEnd.Fragments.ATPPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onSelectvarian: function (e) {
			...
			this.selectedTab = "keyEndingStockView";
			if (e) {
		...
		onPersonlizationClose: function () {
			...
			if (!this.FilterPersonalization) {
				this.FilterPersonalization = sap.ui.xmlfragment("dksh.connectclient.InventoryEnd.Fragments.FilterPersonalization", this);
			...
			e._getPersonalizationDetails("keyEndingStockView")
		},
		...
		valueHelpRequestPlant: function (e) {
			...
			if (t.plantDataAccess === "No Access") {
				...
				var t = this;
				if (!t.plantFrag) {
					t.plantFrag = sap.ui.xmlfragment("dksh.connectclient.InventoryEnd.Fragments.plant", t);
		...
		valueHelpRequestMaterialGrp4: function (e) {
			...
			if (t.materialGroup4DataAccess === "No Access") {
				...
				if (!t.materialGroup4) {
					t.materialGroup4 = sap.ui.xmlfragment("dksh.connectclient.InventoryEnd.Fragments.materialGroup4", t);
		...
		valueHelpRequestMaterialGrp: function (e) {
			...
			if (t.materialGroupDataAccess === "No Access") {
				...
				if (!t.MaterialGroup) {
					t.MaterialGroup = sap.ui.xmlfragment("dksh.connectclient.InventoryEnd.Fragments.MaterialGroup", t);
		...
		valueHelpRequestStoLoc: function (e) {
			...
			if (l.SLOCDataAccess === "No Access") {
				...
				l.StorageLocation = sap.ui.xmlfragment("dksh.connectclient.InventoryEnd.Fragments.StorageLocation", l);
		...
		onVariantDelete: function () {
			...
			var o = {
				varaiantObject: r,
				userId: t,
				applicationId: "keyEndingStockView",
		...
		onExportEndingStock: function () {
			...
			for (var i = 0; i < data.length; i++) {
				data[i].expireDateReport = o.date(data[i].expireDate);
				data[i].manufDateReport = o.manufDate(data[i].manufDate);
			...
			for (var s = 0; s < r.length; s++) {
				if (r[s].status === true) {
					if (r[s].enabledKey === "Material Group") {
						...
						a.push(l)
							// else if (r[s].enabledKey === "SLOC Desc") {
							// 	var l = {
							// 		label: "Storage Location Desc",
							// 		property: "storageLocationDesc"
							// 	};
		...
		popOver: function (e) {
			...
			if (!this._oPopover) {
				r.load({
					name: "dksh.connectclient.InventoryEnd.Fragments.popoverTable",
					controller: this
				}).then(function (e) {
					this._oPopover = e;
					this.getView().addDependent(this._oPopover);
		...
		navigateStkLot: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryEnd.Fragments.StockLotPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		valueHelpRequestSalesOrg: function (e) {
			...
			if (t.salesOrgDataAccess === "No Access") {
				...
				if (!t.salesOrg) {
					t.salesOrg = sap.ui.xmlfragment("dksh.connectclient.InventoryEnd.Fragments.SalesOrg", t);
